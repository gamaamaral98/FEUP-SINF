...
const parseWarehouses = async (data) => {
    ...
    for(let i = 0; i < data.length; i++) {
        if(data[i].warehouseKey !== "A0")
            ret.data.push(data[i].description);
    }
    return ret;
}
    ...
    for(let i = 0; i < data.length; i++){
        ...
        for(let k = 0; k < data[i].materialsItemWarehouses.length; k++){
            ...
            if(stockBalance !== 0){
                ...
                let warehouseDescription = data[i].materialsItemWarehouses[k].warehouseDescription;
                ret.data.push([itemID, description, targetWarehouse, stockBalance, warehouseDescription]);
            }
        }
    }
